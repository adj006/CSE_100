...
public:
  ...
  virtual bool insert(const Data& item) {
    ...
    else
    {
      ...
      {
        // Check whether item is less than the current data
        if( item < curr->data )
        {
          if( curr->left )   // Verify a left child
          {
            //Set the current node travel to the left side
            curr = curr->left;
          }
          else               //Verify if there is no left child 
          {
            //Insert the item to the left child
            curr->left = new BSTNode<Data>( item );
            curr->left->parent = curr;   //Point the left child to it's parent  
            isize++;     
            return true;
          }      
        }
        // Check whether item is greater than the current data
        else if( curr->data < item )
        {
          if( curr->right )	// Verify a right child
          {
            //Set the current node travel to the right side
            curr = curr->right;
          }
          else 			//Verify if there is no left child 
          {
            ...
            curr->right->parent = curr;	//Point the right child to it's parent 
            isize++;
            return true;
          }
        }
